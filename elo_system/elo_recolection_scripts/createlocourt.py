...
games=pd.read_csv("TENNIS32V3.csv",low_memory=False)
players=pd.read_csv("eloCourtV10.csv",low_memory=False)
...
if __name__ == "__main__":
    for z in range(62000,len(games['ID1'])):
        print(str(z)+" de : 162000")
        ...
        if games['COURT'][z]=='Grass':
            grasswin=elo(elo_actualwin['grass'],expected(elo_actualwin['grass'],elo_actuallose['grass']), 1, k=32)
            grasslose=elo(elo_actuallose['grass'],expected(elo_actuallose['grass'],elo_actualwin['grass']),0, k=32)
            players.ix[posicionwin,'grass']=grasswin
            players.ix[posicionloser,'grass']=grasslose
        players.to_csv('eloCourtV10.csv',index=False)