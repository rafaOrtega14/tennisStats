...
def getprediction(datax,datay,player1,player2):
    ...
    model = pickle.load(open('prueba_azul_5.sav', 'rb'))
    players = [{'ACES_1':datax['ACES'],'ACES_2':datay['ACES'],'BPOF_1':datax['BPOF'],
    'BPOF_2':datay['BPOF'],'BP_1':datax['BP'],'BP_2':datay['BP'],'DF_1':datax['DF'],
    'DF_2':datay['DF'],'FSOF_1':datax['FSOF'],'FSOF_2':datay['FSOF'],'FS_1':datax['FS'],
    'FS_2':datay['FS'],'RPWOF_1':datax['RPWOF'],'RPWOF_2':datay['RPWOF'],
    'RPW_1':datax['RPW'],'RPW_2':datay['RPW'],'TPW_1':datax['TPW'],'TPW_2':datay['TPW'],
    'W1SOF_1':datax['W1SOF'],'W1SOF_2':datay['W1SOF'],'W1S_1':datax['W1S'],
    'W1S_2':datay['W1S'],'WIS_1':datax['WIS'],'WIS_2':datay['WIS'],'eloLoser':elos['elop2'],
    'eloWinner':elos['elop1'],'ACESRival_1':datax['ACESRival'],
    'ACESRival_2':datay['ACESRival'],'BPOFRival_1':datax['BPOFRival'],'BPOFRival_2':datay['BPOFRival'],
    'BPRival_1':datax['BPRival'],'BPRival_2':datay['BPRival'],'DFRival_1':datax['DFRival'],
    'DFRival_2':datay['DFRival'],'FSOFRival_1':datax['FSOFRival'],'FSOFRival_2':datay['FSOFRival'],
    'FSRival_1':datax['FSRival'],'FSRival_2':datay['FSRival'],
    'RPWOFRival_1':datax['RPWOFRival'],'RPWOFRival_2':datay['RPWOFRival'],
    'RPWRival_1':datax['RPWRival'],'RPWRival_2':datay['RPWRival'],'TPWRival_1':datax['TPWRival'],
    'TPWRival_2':datay['TPWRival'],'W1SOFRival_1':datax['W1SOFRival'],
    'W1SOFRival_2':datay['W1SOFRival'],'W1SRival_1':datax['W1SRival'],'W1SRival_2':datay['W1SRival'],
    'WISRival_1':datax['WISRival'],'WISRival_2':datay['WISRival']}]
    ...
    print(proba)
    if proba[0][0]>0.5:
        prediction='<b>'+'Probability: '+str(proba[0][0]*100)+'/100'+' W'+str(Player1_rank)+" "+player1+'</b>'+" - "+str(Player2_rank)+" "+player2+' L'+"\n"
    else:
        prediction='<b>'+'Probability: '+str((1-proba[0][0])*100)+'/100'+"</b> "+str(Player1_rank)+" "+player1+' L'+' - '+'<b>'+'W '+str(Player2_rank)+" "+player2+'</b>'+"\n"
    return prediction
def getStatsAndRun():
    players=getPlayers()
    message=""
    elomessage=""
    url = URL + "sendMessage?text={}&chat_id={}&parse_mode=html".format("--------INICIO DE LA PREDICCION--------",'-1001271840201')
    get_url(url)
    for j in players:
        player1=""
        player2=""
        ...
        if (pos1>-1 and pos2>-1):
            ...
            url = URL + "sendMessage?text={}&chat_id={}&parse_mode=html".format(message,'-1001271840201')
            get_url(url)
...
def main():
    getStatsAndRun()
    url = URL + "sendMessage?text={}&chat_id={}&parse_mode=html".format("--------FIN DE LA PREDICCION--------",'-1001271840201')
    get_url(url)
